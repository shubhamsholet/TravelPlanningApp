...
Please ask something travel-related \u{1F30D}`;let d=Object.keys(this.destinationResponses).find(v=>e.includes(v.toLowerCase()));if(d)return this.destinationResponses[d];let c=this.unsupportedDestinations.find(v=>e.includes(v.toLowerCase()));return c?`Sorry \u{1F605} Currently we don't have travel plans or guides for ${c.charAt(0).toUpperCase()+c.slice(1)} yet.
